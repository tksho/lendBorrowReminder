 ...
 * @author   Yugo Kimura <yugon510@hotmail.com>
...
class JWT
{
     ...
     *
     * @param object|array $header  PHP object or array JOSE Header
     * @param object|array $payload PHP object or array JWT Claims Set
     ...
     */
    public static function encode($header, $payload, $key) {
	if( isset($header['alg']) ) {
		$algo = $header['alg'];
	}
        $segments = array();
     ...
     */
    public static function verify($token, $signature, $key, $method = 'RS256') {
        if (empty(self::$methods[$method])) {
            throw new DomainException('Algorithm not supported');
        }
        list($function, $algo) = self::$methods[$method];
        switch($function) {
            case 'openssl':
                $success = openssl_verify($token, $signature, $key, $algo);
                if(!$success) {
			print "this";
            ...
            case 'hash_hmac':
            default:
                $hash = hash_hmac($algo, $token, $key, true);
